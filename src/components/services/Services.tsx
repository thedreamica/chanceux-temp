...
const Services = () => {
  return (
    <section className="px-6 py-5 sm:px-52 sm:py-10">
      <h3 className="text-white text-3xl sm:text-4xl font-bold capitalize mb-6 sm:mb-12 text-center">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-6">
        {data.map((item) => {
          ...
          return (
            ...
            >
              <div className="p-4 bg-gradient-to-r from-indigo-800 to-indigo-400 w-fit rounded-lg mb-6 sm:mb-12">
                {RenderIcon({type: iconType, ...iconProps})}
              </div>
              <h5 className="text-white text-2xl font-bold capitalize mb-2">
                {title}
              </h5>
              <p className="text-xs sm:text-sm text-white leading-snug mb-2 font-medium">{desc}</p>
